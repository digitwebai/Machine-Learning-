...
# Input for dynamic budget range and step
min_budget = st.number_input("Enter minimum budget to test", min_value=0, value=10)
max_budget = st.number_input("Enter maximum budget to test", min_value=0, value=25)
step_budget = st.number_input("Enter step size for budget", min_value=1, value=5)

# Generate the list of budgets to test based on user input
budgets_to_test = list(range(min_budget, max_budget + 1, step_budget))

# Initialize variables
best_budget = None
max_conversions = -1

# Button to trigger the recommendation calculation
